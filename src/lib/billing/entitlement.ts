...
export async function initBilling(): Promise<void> {
	...
	store.when()
		.productUpdated(() => {
			...
			_unlocked = owned;
			if (owned) void persistEntitlement(true);
...
// Triggers the StoreKit purchase sheet. Resolves when ownership is confirmed
// (the approved → verified listener has set _unlocked = true). A user
// dismissing the sheet is treated as a no-op — the caller can check
// isPdfUnlocked() to distinguish "purchased" from "cancelled".
